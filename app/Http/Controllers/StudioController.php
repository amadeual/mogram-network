...
class StudioController extends Controller
{
    public function index()
    {
        ...
        $postRevenue = DB::table('purchases')->whereIn('post_id', $postIds)->sum(DB::raw('amount - commission'));
        ...
        if ($filterType == 'all' || $filterType == 'content') {
            foreach ($postSales as $sale) {
                $history->push([
                    ...
                    'amount' => $sale->amount - (float)($sale->commission ?? 0),
                    'direction' => 'in',
        ...
        if ($filterType == 'all' || $filterType == 'communities' || $filterType == 'subscriptions') {
            foreach ($communitySubs as $sub) {
                $history->push([
                    ...
                    'amount' => $sub->amount - (float)($sub->commission ?? 0),
                    'direction' => 'in',
                    'date' => $sub->created_at,
                    'status' => 'Ativo',
                ]);
