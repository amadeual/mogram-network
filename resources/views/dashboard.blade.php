    ...
    <aside class="right-sidebar">
        ...
        <div>
            ...
            <div style="display: flex; flex-direction: column; gap: 1.5rem;">
                @foreach($trendingUsers as $u)
                <div style="display: flex; align-items: center; justify-content: space-between;">
                    <div style="display: flex; align-items: center; gap: 1rem;">
                        <img src="{{ $u->avatar ? Storage::url($u->avatar) : 'https://api.dicebear.com/7.x/initials/svg?seed=' . $u->name }}" style="width: 36px; height: 36px; border-radius: 50%; border: 1.5px solid rgba(255,255,255,0.05);">
                        <div>
                            <h4 style="font-size: 13px; font-weight: 700;">{{ $u->name }}</h4>
                            <p style="font-size: 11px; color: var(--text-muted);">{{ $u->category ?? 'Sugestão' }}</p>
                        </div>
                    </div>
                    <button onclick="toggleFollowDiscovery(this, '{{ $u->id }}')" style="background: transparent; border: none; font-size: 12px; color: var(--primary-blue); font-weight: 700; text-decoration: none; cursor: pointer;">Seguir</button>
                </div>
                @endforeach
            </div>
        </div>
    ...
    function toggleFollowDiscovery(btn, userId) {
        ...
        .then(data => {
            if (data.status === 'followed') {
                ...
                btn.style.color = 'var(--text-muted)';
                showToast('Agora você está seguindo!');
            } else {
